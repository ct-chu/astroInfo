...
def plot_boundary():
    ...
    lc_boundary = mc.LineCollection(boundary_line_list, colors=[1,0.5,0,0.15],alpha=plot_alpha/2, zorder=1+2.5)
    ax0.add_collection(lc_boundary)
 
def write_label():
    print('timestamp')
    
    ax0.annotate('N '+str(Obs.lat),(hori_xmin,hori_ymin+10),color='w')
    ax0.annotate('E '+str(Obs.lon),(hori_xmin,hori_ymin),color='w')
    ...
    try:
        ...
        urlretrieve(str(re.search('(?P<url>http?://[^\s]+(png|jpeg|jpg))', feed.entries[0]['summary']).group("url")), 'weather_icon.png')
    ...
    #UV
    try:
        HK_UV = re.search(r'(.*)Intensity of UV radiation : (.*?) .*', feed.entries[0]['summary']).group(2).replace('<br/>','')
...
def refresh_sky(i):
    ...
    print('latest update at '+str(datetime.now().time().replace(microsecond=0)))
...
print('backend is '+str(matplotlib.get_backend()))
