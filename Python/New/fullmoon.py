...
DT_UTC1 = ts.utc(2051,9,1)
DT_HK1 = DT_UTC1.astimezone(tz)
DT_UTC2 = ts.utc(2053,10,8)
DT_HK2 = DT_UTC2.astimezone(tz)
...
#list fullmoon
t,y = almanac.find_discrete(DT_UTC1, DT_UTC2, almanac.moon_phases(ephem))

for i in range(len(y)):
    if y[i] == 2:
        (alt_m,az_m,d_m) = Obs.at(t[i]).observe(moon).apparent().altaz()
        print(t.astimezone(tz)[i].strftime('%m/%d/%Y, %H:%M:%S'),format(alt_m.degrees,'.2f'),format(az_m.degrees,'.2f'))
