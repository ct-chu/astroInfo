...
def plot_solar():
    ...
    for i in range(len(equator)-1):
        if (equator['x'].tolist()[i])**2 < (hori_border/2)**2-(equator['y'].tolist()[i])**2:
    ...
    for i in range(len(ecliptic)-1):
        if (ecliptic['x'].tolist()[i])**2 < (hori_border/2)**2-(ecliptic['y'].tolist()[i])**2:
...
def plot_constellation():
    ...
    rect = plt.Rectangle((hori_xmin+shift_mag,hori_ymin), 70, 25, facecolor='black',zorder=13+2.5)
    ax0.add_patch(rect)
...
def plot_boundary():
    ...
    for i in range(len(boundary)-1):
        if (boundary.x[i])**2 < (hori_border/2)**2-(boundary.y[i])**2 and \
           (boundary.x[i+1])**2 < (hori_border/2)**2-(boundary.y[i+1])**2 and \
...
def write_label(x,y,z,c1,c2,c3): # (x,y) are text offset, z is zorder, (c1,c2,c3) are text colors
    ...
    ax0.annotate('astronomical computations by Skyfield',(360+x,hori_ymin+y),rotation=90,ha='right',va='bottom',color=c2,zorder=12+2.5+z)
